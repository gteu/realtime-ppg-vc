...
if ($^O =~ /MSWin/){
	...
	# $juliusbin="/opt/julius-4.3.1/bin/julius";
	$juliusbin="/opt/julius/bin/julius";
}

## acoustic model
$hmmdefs="/work/goto/20210211_ppgvc/RealtimePPGVC/utils/segmentation-kit/models/hmmdefs_monof_mix16_gid.binhmm"; # monophone model
...
#### read the directory
@files = ();
opendir $dh, $wavdir or die "$!:$wavdir";
while ($f = readdir $dh) {
...
foreach $f (@files) {
    ...
    open(TRANS, "$tf.txt") || die "Error: cannot open transcription file \"$tf.txt\"\n";
    # open(TRANS, "$tf.fpl") || die "Error: cannot open fullphone file \"$tf.fpl\"\n";
    while(<TRANS>) {
	chomp;
	next if /^[ \t\n]*$/;
    # push(@words," $_");
    ...
    unlink("$tf.dfa") if (-r "$tf.dfa");
    unlink("$tf.dict") if (-r "$tf.dict");
    open(DFA, "> $tf.dfa") || die "Error: cannot open $tf.dfa for writing\n";
    for ($i = 0; $i <= $num; $i++) {
    ...
    print DFA "$str";
    close(DFA);
    open(DICT, "> $tf.dict") || die "Error: cannot open $tf.dict for writing\n";
    for ($i = 0; $i <= $num; $i++) {
	$w = shift(@words);
	$str = "$i [w_$i] $w\n";
	$WLIST{"w_$i"} = "$w";
    ...
    close(DICT);
    if ((! -r "$tf.dfa") || (! -f "$tf.dfa")) {
	die "Error: failed to make \"$tf.dfa\"\n";
    }
    if ((! -r "$tf.dict") || (! -f "$tf.dict")) {
	die "Error: failed to make \"$tf.dict\"\n";
    }

    #### execute Julius and store the output to log
    $command = "echo $wf.wav | $juliusbin -h $hmmdefs -dfa $tf.dfa -v $tf.dict";
    $command .= " -palign";
    if ($hlist ne "") {
	$command .= " -hlist $hlist";
    }
    ...
    open(RESULT, "> $lf.lab") || die "Error: cannot open result file \"$lf.lab\"\n";
    open(LOG, "$tf.log") || die "Error: cannot open $tf.log file\n";
    $sw = 0;
    while(<LOG>) {
	chomp;
