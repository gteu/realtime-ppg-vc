...
with open("f0_jvs.json") as f:
    target_f0_dict = json.load(f)
...
def test(file_id, recog_model, gen_model, device, X_mean, X_scale, Y_mean, Y_scale):
    ...
    f0_rate = target_f0_dict[config.VC_SPEAKER] / f0[np.nonzero(f0)].mean()
    ...
    spk_onehot = F.one_hot(torch.tensor(int(config.VC_SPEAKER[-3:])-1), num_classes=config.N_SPEAKERS).float().unsqueeze(0).to(device)
    ...
    for i in range(buf_nums):
        ...
        output = gen_model(output, spk_onehot)
    ...
    os.makedirs(os.path.join("gen_multi", config.VC_SPEAKER), exist_ok=True)
    output_wav_path = os.path.join("gen_multi", config.VC_SPEAKER, file_id.split("/")[1].replace(".npy", ".wav"))
...
def main():
    ...
    gen_model_path = glob.glob(os.path.join(config.VC_INFERENCE_MULTI_ROOT, "*best.pth"))[0]
    ...
    if config.SCALE_PER_SPK:
        vc_scaler_path = os.path.join(config.VC_DATA_DIR_PATH, "scalers", config.VC_SPEAKER, "scaler.pkl")
    else:
    ...
    os.makedirs(os.path.join("gen", config.VC_SPEAKER), exist_ok=True)
