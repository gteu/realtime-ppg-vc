...
def train(train_loader, recog_model, gen_model, criterion, optimizer):
    gen_model.train()
...
def main():
    ...
    recog_model_path = glob.glob(os.path.join(config.INFERENCE_ROOT, "*best.pth"))[0]
    ...
    for epoch in range(1, config.VC_N_EPOCHS + 1):
        train_loss = train(train_loader, recog_model, gen_model, criterion, optimizer)
